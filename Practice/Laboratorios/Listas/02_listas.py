...
for name in ['John Lennon', 'Paul McCartney', 'George Harrison']:
    beatles.append(name)
num_miembro = int(input('Digite el número de miembros nuevos: '))

for i in range(1, num_miembro + 1):
    miembros_usuario = input(f'Digite el nombre del miembro nuevo número {i}: ')
    beatles.append(miembros_usuario)

print(f'Lista nueva con datos user: {beatles}')

del beatles[3:]
print(f'Lista nueva con datos eliminados: {beatles}')

beatles.insert(0, 'Ringo Starr')
print(f'Lista nueva con datos ingresados (Primer dato): {beatles}')
